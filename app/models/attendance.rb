class Attendance < ActiveRecord::Base
  ...
  after_save :notify_attendance
  after_destroy :notify_attendances
  ...
  def notify_attendances
    return if self.level_id == Level.find_by_name(:pending).id
    # 1つ前の参加者のレベルを変更する
    prev = Attendance.where(event_id: self.event_id).where('id > ?', self.id).order('id asc').first
    if prev.present?
      prev.accept
    end
  end
  ...
  def check_master_level
    ...
    unless self.level_id == master_level_id
      return self.event.attendances
        .where(level_id: master_level_id)
        .where('attendances.id NOT IN (?)', self.id)
        .exists?
    end
    true
  end
